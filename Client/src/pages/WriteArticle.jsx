      ...
      <div className='border-b border-white/10 bg-[#0F0F12] px-4 py-3 flex-shrink-0'>
        ...
        <form onSubmit={onSubmitHandler} className='flex flex-col gap-2'>
          <div className='flex gap-2 items-center'>
            ...
            <button disabled={loading} className='flex items-center gap-1.5 px-4 py-2 rounded-lg bg-gradient-to-r from-[#226BFF] to-[#65ADFF] text-white text-sm font-medium disabled:opacity-50 whitespace-nowrap flex-shrink-0'>
              {loading ? <><span className='w-4 h-4 rounded-full border-2 border-t-transparent animate-spin'></span><span className='hidden sm:inline'>Generating...</span></> : <><Edit className='w-4 h-4' /><span className='hidden sm:inline'>Generate</span></>}
            </button>
          </div>
          <div className='flex gap-1.5'>
            {articleLength.map((item, i) => (
              <button type='button' key={i} onClick={() => setSelectedLength(item)}
                className={`px-3 py-1.5 rounded-lg text-xs font-medium border transition-all ${selectedLength.text === item.text ? 'bg-blue-500/20 text-blue-400 border-blue-500/40' : 'text-gray-400 border-white/10 hover:border-white/20'}`}>
                {item.text}
              </button>
            ))}
          </div>
