...
import { Heart, X, ImageIcon, Calendar } from 'lucide-react'
...
const ProfileModal = ({ userId, onClose, getToken }) => {
                    ...
                    <>
                        ...
                        <div className='grid grid-cols-2 gap-3 mb-6'>
                            <div className='bg-white/5 border border-white/10 rounded-xl p-4 text-center'>
                                <div className='flex items-center justify-center gap-1.5 text-blue-400 mb-1'><ImageIcon className='w-4 h-4' /></div>
                            ...
                            <div className='bg-white/5 border border-white/10 rounded-xl p-4 text-center'>
                                <div className='flex items-center justify-center gap-1.5 text-red-400 mb-1'><Heart className='w-4 h-4' /></div>
                            ...
                            <>
                                ...
                                <div className='grid grid-cols-3 gap-2 max-h-48 overflow-y-auto'>
                                    {profile.creations.map((c, i) => (
                                        <img key={i} src={c.content} alt={c.prompt} title={c.prompt}
...
// ── Single card with tap-to-reveal prompt on mobile ──
const CreationCard = ({ creation, user, onLike, onProfileClick }) => {
    const [showPrompt, setShowPrompt] = useState(false)

    return (
        <div className='relative inline-block pl-3 pt-3 w-full sm:w-1/2 lg:w-1/3'>
            <div className='relative rounded-lg overflow-hidden'>
                ...
                <div className='absolute bottom-0 left-0 right-0 bg-gradient-to-t from-black/80 to-transparent px-3 py-2 flex items-end justify-between'>
                    <button
                        onClick={() => onProfileClick(creation.user_id)}
                        ...
                        {creation.creator_avatar ? (
                            ...
                            <div className='w-6 h-6 rounded-full bg-purple-500/40 border border-purple-400/30 flex items-center justify-center flex-shrink-0'>
                                <span className='text-white text-[10px] font-semibold'>{creation.creator_name?.[0]?.toUpperCase() || '?'}</span>
                            </div>
                        )}
                        <span className='text-white text-xs font-medium truncate max-w-[100px]'>{creation.creator_name}</span>
                    </button>
                ...
                {/* Prompt overlay:
                    - Desktop: show on hover via CSS group
                    - Mobile: show on tap via state toggle */}
                <div
                    onClick={() => setShowPrompt(v => !v)}
                    className={`absolute inset-0 bg-black/60 flex items-center justify-center p-4 transition-opacity cursor-pointer
                ...
                <div className='absolute top-2 right-2 sm:hidden bg-black/50 rounded-full px-2 py-0.5 pointer-events-none'>
                    <p className='text-[10px] text-white/60'>tap for prompt</p>
                </div>
            </div>
        </div>
    )
...
const Community = () => {
    ...
    return !loading ? (
        <div className='flex-1 h-full flex flex-col gap-4 p-6 bg-[#0A0A0D]'>
            {selectedUserId && (
                <ProfileModal userId={selectedUserId} onClose={() => setSelectedUserId(null)} getToken={getToken} />
            )}
