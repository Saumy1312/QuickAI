        ...
        <form onSubmit={onSubmitHandler} className='flex flex-col gap-2'>
          <div className='flex gap-2 items-center'>
            <input onChange={(e) => setInput(e.target.value)} value={input} type="text"
              className='flex-1 px-3 py-2 rounded-lg bg-white/5 border border-white/10 outline-none text-sm text-white placeholder-gray-600 focus:border-green-500/50 transition-colors'
            ...
            <label className='flex items-center gap-1.5 cursor-pointer flex-shrink-0'>
              ...
              <span className='text-xs text-gray-400 whitespace-nowrap'>Public</span>
            </label>
            <button disabled={loading} type='submit' className='flex items-center gap-1.5 px-4 py-2 rounded-lg bg-gradient-to-r from-[#00AD25] to-[#04FF50] text-white text-sm font-medium disabled:opacity-50 whitespace-nowrap flex-shrink-0'>
              {loading ? <><span className='w-4 h-4 rounded-full border-2 border-t-transparent animate-spin'></span><span className='hidden sm:inline'>Generating...</span></> : <><Image className='w-4 h-4' /><span className='hidden sm:inline'>Generate</span></>}
            </button>
          </div>
