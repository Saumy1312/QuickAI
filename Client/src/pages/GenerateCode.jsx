import { Code, Send, Plus, Pencil, Trash2, Check, X, Menu } from 'lucide-react'
...
const GenerateCode = () => {
  ...
  const onSubmitHandler = async (e) => {
    ...
    try {
      setLoading(true)
      const { data } = await axios.post('/api/ai/generate-code', { prompt: input, sessionId: currentSessionId }, { headers: { Authorization: `Bearer ${await getToken()}` } })
    ...
    <div className='h-full flex text-white bg-[#0A0A0D] overflow-hidden'>
      ...
      <div className='flex-1 flex flex-col overflow-hidden min-w-0'>
        <div className='flex items-center gap-3 px-4 py-3 border-b border-white/10 bg-[#0F0F12] flex-shrink-0'>
          ...
          <h1 className='text-base font-semibold'>Code Generator</h1>
        </div>
        ...
        <div className='flex-1 overflow-y-auto p-3 sm:p-4 flex flex-col gap-3'>
          {messages.length === 0 && (
            <div className='flex-1 flex flex-col justify-center items-center text-gray-600 gap-3 h-full'>
              ...
              <p className='text-sm text-center px-4'>Ask me to write any code for you!</p>
            </div>
          )}
          {messages.map((msg, i) => (
            <div key={i} className={`flex ${msg.role === 'user' ? 'justify-end' : 'justify-start'}`}>
              <div className={`max-w-[88%] sm:max-w-[75%] px-3 py-2.5 rounded-xl text-sm leading-relaxed ${msg.role === 'user' ? 'bg-emerald-600/20 border border-emerald-500/30 text-emerald-100 rounded-br-sm' : 'bg-[#0F0F12] border border-white/10 text-gray-300 rounded-bl-sm'}`}>
        ...
        <form onSubmit={onSubmitHandler} className='p-3 border-t border-white/10 bg-[#0F0F12] flex gap-2 flex-shrink-0'>
          <input type='text' value={input} onChange={e => setInput(e.target.value)} placeholder='Describe the code you need...'
            className='flex-1 px-3 py-2.5 rounded-xl bg-white/5 border border-white/10 outline-none text-sm text-white placeholder-gray-500 focus:border-emerald-500/50 transition-colors min-w-0' required />
          <button disabled={loading} className='flex items-center justify-center w-10 h-10 rounded-xl bg-emerald-600 hover:bg-emerald-500 disabled:opacity-50 text-white transition-colors flex-shrink-0'>
