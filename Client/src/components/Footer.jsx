...
const Footer = () => {
  ...
  const handleSubscribe = () => {
    ...
    // You can replace this with an actual API call e.g. axios.post('/api/subscribe', { email })
  ...
  return (
    <footer id="contact" className='bg-[#0A0A0D] border-t border-white/10 text-gray-400 px-6 md:px-16 lg:px-24 xl:px-32 pt-10 pb-6'>
      <div className='flex flex-col md:flex-row md:justify-between items-center md:items-start text-center md:text-left gap-8'>
        ...
        <div className='text-sm flex flex-col gap-2'>
          {navLinks.map(({ label, id }) => (
            <button
              key={label}
              onClick={() => scrollTo(id)}
              className='text-gray-500 hover:text-white transition-colors duration-200 text-left'
            >
              {label}
            </button>
          ))}
        ...
        <div className='w-full max-w-xs'>
          ...
          <div className='flex'>
            ...
            <button
              ...
              className='bg-white text-black px-4 h-9 text-sm rounded-r font-medium hover:bg-gray-100 transition'
            >
              Send
            </button>
          </div>
        ...
        <div className='flex gap-4 items-center text-gray-500'>
          {/* Instagram */}
          <a href="https://www.instagram.com/saumy1312/" target="_blank" rel="noopener noreferrer" aria-label="Instagram" className='hover:text-white transition-colors'>
            <svg className='w-5 h-5' fill='currentColor' viewBox='0 0 24 24'>
              <path d="M7.75 2A5.75 5.75 0 002 7.75v8.5A5.75 5.75 0 007.75 22h8.5A5.75 5.75 0 0022 16.25v-8.5A5.75 5.75 0 0016.25 2h-8.5zM4.5 7.75A3.25 3.25 0 017.75 4.5h8.5a3.25 3.25 0 013.25 3.25v8.5a3.25 3.25 0 01-3.25 3.25h-8.5a3.25 3.25 0 01-3.25-3.25v-8.5zm9.5 1a4 4 0 11-4 4 4 4 0 014-4zm0 1.5a2.5 2.5 0 102.5 2.5 2.5 2.5 0 00-2.5-2.5zm3.5-.75a.75.75 0 11.75-.75.75.75 0 01-.75.75z" />
            </svg>
          </a>
          {/* LinkedIn */}
          <a href="https://www.linkedin.com/in/saumy-singh-0337832a6/" target="_blank" rel="noopener noreferrer" aria-label="LinkedIn" className='hover:text-white transition-colors'>
            <svg className='w-5 h-5' fill='currentColor' viewBox='0 0 24 24'>
              <path d="M4.98 3.5C3.88 3.5 3 4.38 3 5.48c0 1.1.88 1.98 1.98 1.98h.02c1.1 0 1.98-.88 1.98-1.98C6.98 4.38 6.1 3.5 4.98 3.5zM3 8.75h3.96V21H3V8.75zm6.25 0h3.8v1.68h.05c.53-.98 1.82-2.02 3.75-2.02 4.01 0 4.75 2.64 4.75 6.07V21H17v-5.63c0-1.34-.03-3.07-1.88-3.07-1.88 0-2.17 1.47-2.17 2.98V21H9.25V8.75z" />
            </svg>
          </a>
          {/* GitHub */}
          <a href="https://github.com/Saumy1312" target="_blank" rel="noopener noreferrer" aria-label="GitHub" className='hover:text-white transition-colors'>
            <svg className='w-5 h-5' fill='currentColor' viewBox='0 0 24 24'>
