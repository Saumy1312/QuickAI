...
const Footer = () => (
  <footer className='bg-[#0A0A0D] border-t border-white/10 px-6 sm:px-16 xl:px-32 pt-12 pb-8'>
    <div className='flex flex-col md:flex-row md:justify-between gap-10'>
      <div className='max-w-xs'>
        <img src={assets.logo} alt="logo" className='h-7 mb-3 brightness-200' />
        <p className='text-gray-500 text-sm'>QuickAI helps you write, design, and innovate smarter — with AI tools built for creators.</p>
      </div>
      <div className='flex flex-col gap-2 text-sm text-gray-500'>
        {['Home','Features','Pricing','Contact'].map(l => <a key={l} href="#" className='hover:text-white transition-colors'>{l}</a>)}
      </div>
      <div className='max-w-xs'>
        <p className='text-sm font-medium text-white mb-2'>Subscribe for updates</p>
        <div className='flex'>
          <input type="email" placeholder='Enter your email' className='flex-1 h-9 px-3 text-sm bg-white/5 border border-white/10 rounded-l outline-none text-white placeholder-gray-600 focus:border-white/20' />
